...
function candidateFromTopic(item, index) {
  return {
    ...
    reason: "기존 생활 정보 주제 큐의 미사용 후보입니다.",
    source: "prompts/info-topics.json"
  };
}
...
const candidateFile = path.join(CANDIDATE_DIR, `${stamp}-info-blog.json`);
const bodyFile = path.join(CANDIDATE_DIR, `${stamp}-info-blog.md`);
const payload = {
...
await writeGitHubOutputs({
  ...
  title: `AI info blog topic candidates ${stamp}`
});
