...
function imagePrompt({ index, title, alt }) {
  return [
    "Create a simple, lightweight informational blog illustration.",
    `Blog topic: ${topic}`,
    `Blog title: ${title || topic}`,
    `Image ${index} purpose: ${alt}`,
    "Style: minimal flat editorial illustration, few everyday objects, simple shapes, fresh light-green accents, clean background.",
    "Prefer an abstract information, checklist, document, calendar, pricing, travel, or service-change concept.",
    "Avoid photorealism, dense details, tiny UI, fake text, logos, brand names, watermarks, and legal-document fine print.",
...
async function generatePostImages(markdown, slug) {
  ...
  for (const index of markers) {
    const alt = altTexts.get(index) || `${topic} 관련 생활 정보 블로그 이미지 ${index}`;
